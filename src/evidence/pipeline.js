...
import { findCountry, CIVIC_CATEGORIES } from './civicScope.js';

const wait = (ms) => new Promise((r) => setTimeout(r, ms));

/** A pack always names its own country; that is enough to suggest where
 *  to search when nothing local matched. Static lookup, no network. */
const scopeFor = (pack) => {
  const country = findCountry(pack?.meta?.country_name ?? pack?.meta?.country ?? '');
  return country ? { country, categories: CIVIC_CATEGORIES } : null;
};
 ...
 * @param {(query:string, context?:string)=>Promise<object>} [opts.search]
 *   Live web search, injected — see tools/index.js ground_current_information.
 */
export async function runVerification({
  pack, utterance = '', image = null, fieldEvidence = [], coarseLocation = null,
  online = false, search = null, onMote = () => {}, pace = 0, signal,
} = {}) {
  const failures = [];
  ...
  if (!ent.ok) {
    /* Law 4: no match is not disproof. Before giving up, a live search is
       a different, lesser kind of help — not a verified finding, never
       dressed up as one, but a real lead with a real link is still more
       useful than a bare refusal when one is reachable. */
    const lead = await step('ground_current_information', () =>
      T.ground_current_information(utterance || clues.map((c) => c.value).join(' '),
        { online, search, context: jur.ok ? jur.admin1 : '' }));
    return {
      ...
      /* ok:true with an empty source list means "checked, found nothing" —
         a real result, just not a renderable one. Only a lead with an
         actual source is worth a card; the plain scope fallback covers
         the rest. */
      lead: (lead.ok && lead.sources?.length) ? lead : null,
      scope: scopeFor(pack),
      failures,
    };
  }
  ...
  /* 4 — records */
  const search_pack = await step('search_country_pack', () => T.search_country_pack(ent.chosen.id, pack));
  if (!search_pack.ok) {
    return { ok: false, kind: 'no_records', reason: search_pack.reason, failures,
  ...
  await step('ground_current_information', () =>
    T.ground_current_information(ent.chosen.name, { online, search, context: ent.chosen.admin1 ?? '' }));
  ...
  const verified = await step('verify_claim', () => T.verify_claim({
    ...
    entity: ent.chosen, records: search_pack.records, sources: search_pack.sources,
...
export async function runChallenge({ payload, pack, online = false, search = null, onMote = () => {}, pace = 0 }) {
  const tool = 'challenge_finding';
  const label = MOTE_LABEL[tool];
  onMote({ type: 'mote', tool, label, status: 'running' });
  if (pace) await wait(pace);
  const res = await T.challenge_finding(payload, pack, { online, search });
